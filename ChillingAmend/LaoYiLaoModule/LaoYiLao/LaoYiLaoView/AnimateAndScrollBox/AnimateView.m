...
#define TitleImageViewY TopHeight * KPercenY
#define TitleImageViewW 242 * KPercenX
#define TitleImageViewH 58 * KPercenY

//饺子和勺view的起始坐标
#define DumplingsAndSpoonBtnX 125 * KPercenX
#define DumplingsAndSpoonBtnY  CGRectGetMaxY(_titleImageView.frame) + 10 * KPercenY
...
- (void)addAnimateView{
    ...
    _titleImageView = [[UIImageView alloc]initWithFrame:CGRectMake(TitleImageViewX, TitleImageViewY, TitleImageViewW, TitleImageViewH)];
    _titleImageView.image = [UIImage imageNamed:@"banner"];
    _titleImageView.hidden = NO;
    _titleImageView.center = CGPointMake(self.frame.size.width / 2, _titleImageView.center.y);
    [self addSubview:_titleImageView];
    
//    _timeLineView = [LYTimelineView createTimelineViewWithDataArray:@[] currentTime:[NSDate date] position:CGPointMake(TitleImageViewX, TitleImageViewY)];
    _timeLineView = [[LYTimelineView alloc]initWithFrame:CGRectMake(TitleImageViewX, TitleImageViewY, kkViewWidth,58 * KPercenY)];
...
//    _panBtn.backgroundColor = [UIColor clearColor];
    NSURL * imgURL = [NSURL URLWithString:PanAdImage];
    [_panBtn sd_setImageWithURL:imgURL forState:UIControlStateNormal];
    [_panBtn sd_setImageWithURL:imgURL forState:UIControlStateHighlighted];
    
//    [_panBtn setImage:[UIImage imageNamed:@"PanAdImage"] forState:UIControlStateNormal];
//    [_panBtn setImage:[UIImage imageNamed:@"PanAdImage"] forState:UIControlStateHighlighted];
 ...
 */
- (void)startTimer{
    [_numTimer setFireDate:[NSDate distantPast]];//开启定时器
}
